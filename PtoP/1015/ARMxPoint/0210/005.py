...
class AnimationController:
    def __init__(self):
        ...
        self.R_p7_total = np.eye(3)  # P7旋轉矩陣,初始化為單位矩陣
    ...
    def get_joint5_position(self):
        """計算關節5末端點位置(P7末端,跟著P7實際旋轉)"""
        # 計算P6末端位置(P7基座)
        p6_end = self.R_p2_total @ (
                self.p3_base_local +
                self.R_p3_total @ (
                        self.p3_length_vec +
                        self.R_p4_total @ (
                                ...
                                self.R_p5_total @ (
                                        self.p5_length_vec +
                                        self.R_p6_total @ self.p6_length_vec
                                )
                        )
                )
        )

        # P7向量在世界座標系中的方向(跟著P7實際旋轉)
        p7_vec_world = self.R_p2_total @ self.R_p3_total @ self.R_p4_total @ self.R_p5_total @ self.R_p6_total @ self.p7_length_vec

        # P7末端 = P6末端 + P7向量
        p7_end = p6_end + p7_vec_world
    ...
    def fk(self, angles):
        """正向運動學:計算關節5末端點位置(P7末端,跟著P7實際旋轉)"""
        ...
        R6 = o3d.geometry.get_rotation_matrix_from_axis_angle([0, np.deg2rad(angles[4]), 0])

        # P6末端位置
        p6_end = R2 @ (self.p3_base_local + R3 @ (
                self.p3_length_vec + R4 @ (
                self.p4_length_vec + R5 @ (
                self.p5_length_vec + R6 @ self.p6_length_vec))))

        # P7向量(跟著實際旋轉)
        p7_vec_world = R2 @ R3 @ R4 @ R5 @ R6 @ self.p7_length_vec
