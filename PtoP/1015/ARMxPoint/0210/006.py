...
14. 紅球位置改為關節5末端點(P7末端),紅球尺寸縮小50%
15. 目標位置預設值改為x0.2 y0.2 z0.7
16. 動畫倍速預設改為10倍
17. 修正最後一點執行問題
18. 紅球永遠固定在P7末端點,手動拉滑桿時會跟著移動
19. 末端點座標顯示紅球即時位置
20. 正向運動學時紅球跟著P7實際旋轉(不強制垂直)
21. 逆向運動學時計算關節5角度使P7垂直
'''
...
class AnimationController:
    def __init__(self):
        ...
        # 初始旋轉:P6(關節5)初始有-90度旋轉
        self.R_p6_total = o3d.geometry.get_rotation_matrix_from_axis_angle([0, np.deg2rad(-90), 0])
        self.R_p7_total = np.eye(3)
    ...
    def get_joint5_position(self):
        """計算關節5末端點位置(P7末端,P7長度為L4)"""
        # 計算P7基座位置(P6末端 + p7位移)
        p7_base = self.R_p2_total @ (
                self.p3_base_local +
                self.R_p3_total @ (
                        self.p3_length_vec +
                        self.R_p4_total @ (
        ...
        # P7方向(跟著實際旋轉),P7長度為L4
        # P7的Z軸方向在局部座標系是向下的
        p7_direction = self.R_p2_total @ self.R_p3_total @ self.R_p4_total @ self.R_p5_total @ self.R_p6_total @ np.array(
            [0, 0, 1])

        # P7末端 = P7基座 + P7方向 * L4
        p7_end = p7_base + p7_direction * self.L4
    ...
    def fk(self, angles):
        ...
        R6_init = o3d.geometry.get_rotation_matrix_from_axis_angle([0, np.deg2rad(-90), 0])
        R6 = o3d.geometry.get_rotation_matrix_from_axis_angle([0, np.deg2rad(angles[4]), 0]) @ R6_init

        # P7基座位置
        p7_base = R2 @ (self.p3_base_local + R3 @ (
                self.p3_length_vec + R4 @ (
                self.p4_length_vec + R5 @ (
                self.p5_length_vec + R6 @ (
                self.p6_length_vec + self.p7_length_vec)))))

        # P7方向,P7長度為L4
        p7_direction = R2 @ R3 @ R4 @ R5 @ R6 @ np.array([0, 0, 1])
    ...
    def ik(self, x, y, z):
        """逆向運動學:從目標點反推關節角度,計算關節5使P7垂直"""
        try:
            # 目標點是P7末端,需要反推到P5末端(關節5基座)
            # 因為我們要讓P7垂直向下,所以P7末端 = P5末端 + [0, 0, -L4]
            # 因此 P5末端Z = P7末端Z + L4
            z_base = z + self.L4
            ...
            # 計算關節5角度,使P7保持垂直
            # theta5 = -(theta2 + theta3) 使P7垂直於XY平面
    ...
    def move_next_point(self):
        ...
        if ok:
            ...
            # 計算下一個索引
            next_index = self.trajectory_index + self.trajectory_step
            ...
            if self.trajectory_index >= len(self.trajectory_points) - 1:
                ...
                self.is_following_trajectory = False
                self.skipped_points_count = 0
            elif next_index >= len(self.trajectory_points):
                # 下一次跳躍會超出,先跳到最後一點
                self.trajectory_index = len(self.trajectory_points) - 1
            else:
                # 正常前進
            ...
            if self.skip_unreachable_points:
                ...
                # 計算下一個索引
                next_index = self.trajectory_index + self.trajectory_step
                if next_index >= len(self.trajectory_points):
                    # 如果跳過後會超出範圍,跳到最後一點
                    self.trajectory_index = len(self.trajectory_points) - 1
                else:
                    self.trajectory_index = next_index
