...
5. 軌跡執行時自動避障
6. 障礙物控制介面移至右側
7. 關節運動時即時碰撞檢測(框線範圍內遇到障礙物會停止並顯示警告)
...
class ArmSegmentBoundingBox:
    ...
    def check_collision_with_box(self, obstacle_center, obstacle_size):
        """
        檢查包圍盒是否與障礙物(AABB)碰撞
        使用分離軸定理(SAT)進行OBB vs AABB碰撞檢測

        obstacle_center: [x, y, z] 障礙物中心
        obstacle_size: [length, width, height] 障礙物尺寸
        返回: True 如果碰撞, False 如果安全
        """
        obstacle_center = np.array(obstacle_center)
        obstacle_half_size = np.array(obstacle_size) / 2

        # 獲取OBB的軸(旋轉後的局部軸)
        obb_axes = self.R_total  # 3x3 旋轉矩陣的每一列是一個軸
        obb_half_size = self.size / 2

        # 計算兩個包圍盒中心的距離向量
        center_distance = obstacle_center - self.current_center
        ...
        for i in range(3):
            ...
            # 投影OBB到當前軸
            obb_projection = sum(abs(np.dot(obb_axes[:, j], axis)) * obb_half_size[j] for j in range(3))
            # 投影AABB到當前軸
            aabb_projection = obstacle_half_size[i]
            # 投影中心距離到當前軸
            distance_projection = abs(np.dot(center_distance, axis))

            if distance_projection > obb_projection + aabb_projection:
        ...
        for i in range(3):
            ...
            # 投影OBB到當前軸
            obb_projection = obb_half_size[i]
            # 投影AABB到當前軸
            aabb_projection = sum(abs(axis[j]) * obstacle_half_size[j] for j in range(3))
            # 投影中心距離到當前軸
            distance_projection = abs(np.dot(center_distance, axis))
        ...
        for i in range(3):
            for j in range(3):
                axis_aabb = np.zeros(3)
                axis_aabb[j] = 1.0
                axis = np.cross(obb_axes[:, i], axis_aabb)
                ...
                axis = axis / axis_length  # 歸一化

                # 投影OBB到當前軸
                obb_projection = sum(abs(np.dot(obb_axes[:, k], axis)) * obb_half_size[k] for k in range(3))
                # 投影AABB到當前軸
                aabb_projection = sum(abs(axis[k]) * obstacle_half_size[k] for k in range(3))
                # 投影中心距離到當前軸
                distance_projection = abs(np.dot(center_distance, axis))
...
class ArmBoundingBoxManager:
    ...
    def check_collision_with_obstacles(self, obstacles):
        ...
        檢查手臂包圍盒是否與任何障礙物碰撞
        obstacles: 障礙物列表
        返回: (is_collision, segment_name, obstacle_name)
        """
        for bbox in self.bounding_boxes:
            for obstacle in obstacles:
                if bbox.check_collision_with_box(obstacle.center, obstacle.size):
                    return True, bbox.name, obstacle.name
...
print("\n" + "=" * 60)
print("RA605-710-GC 六軸機械手臂控制系統 (障礙物避障版+關節碰撞檢測)")
print("=" * 60)
...
class AnimationController:
    def __init__(self, obstacle_manager, arm_bbox_manager):
        ...
        # 關節運動碰撞檢測標誌
        self.enable_joint_collision_check = True
        self.joint_motion_blocked = False
    ...
    def check_arm_obstacle_collision(self):
        """
        檢查手臂包圍盒是否與障礙物碰撞
        返回: (is_collision, segment_name, obstacle_name)
        """
        if not self.enable_joint_collision_check:
            return False, None, None

        return self.arm_bbox_manager.check_collision_with_obstacles(
    ...
    def update_joint(self, joint_idx, delta_angle):
        ...
        # 檢查碰撞(關節運動時)
        if self.enable_joint_collision_check and not self.is_following_trajectory:
            is_collision, segment_name, obstacle_name = self.check_arm_obstacle_collision()
            if is_collision:
                self.joint_motion_blocked = True
                self.is_animating = False  # 停止動畫
                self.log_status(f"⚠️ 警告: {segment_name} 與 {obstacle_name} 碰撞!\n框線範圍內遇到障礙物,已停止移動")
    ...
    def start_trajectory(self, points):
        ...
        self.joint_motion_blocked = False  # 清除碰撞阻塞
...
class RobotControlGUI:
    def __init__(self, root, controller, obstacle_manager):
        ...
        self.root.title("RA605-710-GC 六軸機械手臂控制系統 (障礙物避障版+關節碰撞檢測)")
        self.root.geometry("850x1050+0+50")
    ...
    def setup_ui(self):
        ...
        main_frame.columnconfigure(0, weight=0)  # 左側固定寬度
        main_frame.columnconfigure(1, weight=1)  # 右側可伸縮
        main_frame.rowconfigure(0, weight=1)  # 上方可伸縮
        main_frame.rowconfigure(1, weight=0)  # 下方固定高度

        self.create_joint_control(main_frame)
        self.create_right_panel(main_frame)  # 建立右側面板(包含位置控制和障礙物控制)
    ...
    def create_joint_control(self, parent):
        ...
        # 碰撞檢測開關
        collision_frame = ttk.Frame(anim_frame)
        collision_frame.pack(fill=tk.X, pady=5)

        self.collision_check_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(collision_frame, text="啟用關節運動碰撞檢測",
                        variable=self.collision_check_var,
                        command=self.toggle_collision_check).pack(anchor=tk.W)
    ...
    def toggle_collision_check(self):
        """切換碰撞檢測"""
        self.ctrl.enable_joint_collision_check = self.collision_check_var.get()
        if self.ctrl.enable_joint_collision_check:
            self.append_status("已啟用關節運動碰撞檢測")
        else:
            self.append_status("已停用關節運動碰撞檢測")
            self.ctrl.joint_motion_blocked = False  # 清除碰撞阻塞
    ...
    def create_right_panel(self, parent):
        ...
        frame.rowconfigure(0, weight=2)  # 位置控制區域(較大)
        frame.rowconfigure(1, weight=1)  # 障礙物控制區域(較小)
        frame.columnconfigure(0, weight=1)

        self.create_position_control(frame)
        self.create_obstacle_control(frame)

    def create_position_control(self, parent):
        """位置控制與軌跡規劃面板"""
        frame = ttk.Frame(parent)
        frame.grid(row=0, column=0, padx=0, pady=(0, 5), sticky=(tk.N, tk.S, tk.W, tk.E))
    ...
    def create_obstacle_control(self, parent):
        ...
        frame.grid(row=1, column=0, padx=0, pady=(5, 0), sticky=(tk.W, tk.E, tk.N, tk.S))
        ...
        # 中心位置
        center_frame = ttk.LabelFrame(param_frame, text="中心位置 (公尺)", padding="5")
        ...
        # 尺寸
        size_frame = ttk.LabelFrame(param_frame, text="尺寸 (公尺)", padding="5")
        size_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(5, 0))
        ...
        list_label = ttk.Label(list_frame, text="當前障礙物列表:", font=('Arial', 9, 'bold'))
        ...
        self.obstacle_listbox = tk.Listbox(list_container, height=4,
                                           font=('Courier', 8),
    ...
    def create_system_control(self, parent):
        """系統控制面板"""
        frame = ttk.Frame(parent)
        frame.grid(row=1, column=0, columnspan=2, pady=10, sticky=(tk.W, tk.E))

        ttk.Button(frame, text="關閉程式",
                   command=self.quit_program).pack(side=tk.RIGHT, padx=5)

        self.info_label = ttk.Label(frame, text="RA605-710-GC 六軸機械手臂 (障礙物避障版+關節碰撞檢測)",
                                    font=('Arial', 9))
