...
class AnimationController:
    def __init__(self):
        ...
        self.trajectory_delay = 0.05  # 點與點之間的延遲時間（秒），預設50ms (速度1x)
    ...
    def move_next_point(self):
        ...
        if ok:
            ...
            if self.trajectory_index % 100 == 0:
                print(f"Progress: {self.trajectory_index}/{len(self.trajectory_points)} (Delay: {self.trajectory_delay*1000:.1f}ms, AnimSpeed: {self.animation_speed:.2f})")
        else:
    ...
    def set_trajectory_speed(self, speed):
        ...
        speed: 1-20，數字越大速度越快
        延遲時間範圍: 50ms(speed=1) 到 2.5ms(speed=20)
        關節動畫速度也會同步提升
        """
        # 調整發送目標點的間隔時間
        self.trajectory_delay = 0.05 / speed
...
def start_gui():
    ...
    def move_arc():
        try:
            ...
            delay_ms = (0.05 / current_speed) * 1000
            status.config(text=f"Arc: {length:.3f}m\nRunning... ({delay_ms:.1f}ms/point)")
            root.update()
    ...
    spd = tk.Scale(lf, from_=5, to=30, orient=tk.HORIZONTAL, length=180,
    ...
    def update_traj_speed(v):
        ...
        delay_ms = (0.05 / speed) * 1000  # 轉換為毫秒
        anim_speed_percent = int(ctrl.animation_speed * 100)
        speed_label.config(text=f"Speed: {int(speed)}x ({delay_ms:.1f}ms, Anim:{anim_speed_percent}%)")
        # 如果正在執行軌跡，更新狀態顯示
    ...
    speed_label = tk.Label(speed_frame, text="Speed: 5x (10ms, Anim:22%)", width=28)
    speed_label.pack()

    traj_speed = tk.Scale(speed_frame, from_=1, to=20, resolution=1,
                          orient=tk.HORIZONTAL, length=180,
                          command=update_traj_speed, showvalue=False)
    traj_speed.set(5)  # 預設速度為5x (10ms/point, 動畫速度提升到0.216)
    traj_speed.pack()

    # 初始化速度顯示
    update_traj_speed(5)

    tk.Label(speed_frame, text="(1=Slowest, 20=Fastest)", font=("Arial", 7)).pack()
