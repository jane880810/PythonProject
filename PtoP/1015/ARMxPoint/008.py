    ...
    def move_next_point(self):
        ...
        if ok:
            ...
            if self.trajectory_index % 100 == 0:
                print(f"Progress: {self.trajectory_index}/{len(self.trajectory_points)} (Delay: {self.trajectory_delay*1000:.2f}ms)")
        else:
...
def start_gui():
    ...
    speed_label = tk.Label(speed_frame, text="Speed: 100x (0.05ms)", width=30)
    speed_label.pack()
    ...
    tk.Label(speed_frame, text="(Range: 1-1000x)", font=("Arial", 7)).pack()
