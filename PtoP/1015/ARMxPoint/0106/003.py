...
class ArmSegmentBoundingBox:
    """手臂連桿包圍盒 - 用於碰撞檢測"""
    def __init__(self, center, size, name="", color=[0.5, 0.5, 0.5, 0.5]):
        """
        初始化手臂連桿包圍盒
        center: [x, y, z] 中心位置 (公尺)
        size: [length, width, height] 尺寸 (公尺)
        name: 連桿名稱
        color: RGBA顏色 (最後一個值是透明度,但Open3D使用半透明顯示)
        """
        ...
        self.color = color[:3]  # RGB only for Open3D
        self.mesh = None
    ...
    def create_mesh(self):
        ...
        # 移動到中心位置
        self.mesh.translate(self.current_center - self.size / 2)
        # 設定半透明灰色
...
if len(arm_bboxes) >= 8:
    ...
    # p8 - 跟隨 node8 旋轉 (末端執行器) - index 7
    node8.set_bounding_box(arm_bboxes[7])
        ...
        if ok:
            ...
            if self.skip_unreachable_points:
                ...
                while self.trajectory_index < len(self.trajectory_points):
                    ...
                    # 檢查下一個點是否與手臂碰撞
                    is_arm_collision_next, arm_segment_name_next = self.arm_bbox_manager.check_collision_with_point(pt_next)
