...
class AnimationController:
    def __init__(self):
        ...
        self.trajectory_speed = 1  # 軌跡速度：每次移動跳過的點數
    ...
    def move_next_point(self):
        ...
        if ok:
            # 根據速度設定跳過點數
            self.trajectory_index += max(1, int(self.trajectory_speed))
...
def start_gui():
    ...
    def move_arc():
        try:
            ...
            status.config(text=f"Arc: {length:.3f}m\nRunning... (Speed: {int(ctrl.trajectory_speed)}x)")
            root.update()
    ...
    def update_traj_speed(v):
        ...
        speed_label.config(text=f"Speed: {int(speed)}x")
        # 如果正在執行軌跡，更新狀態顯示
    ...
    tk.Label(speed_frame, text="(1=Slow, 20=Fast)", font=("Arial", 7)).pack()
