...
class AnimationController:
    def __init__(self):
        ...
        self.animation_speed = 0.12  # 關節動畫速度（基礎值）
        self.base_animation_speed = 0.12  # 保存基礎動畫速度
        ...
        self.trajectory_delay = 0.005  # 點與點之間的延遲時間（秒），預設5ms (速度1x)
    ...
    def start_trajectory(self, points):
        ...
        self.trajectory_index = 0
        self.skipped_points_count = 0  # 重置跳過計數
    ...
    def move_next_point(self):
        ...
        ok, msg = self.move_to(pt[0], pt[1], pt[2])

        if ok:
            # 成功移動到該點
            self.trajectory_index += 1
            if self.trajectory_index % 100 == 0:
                print(f"Progress: {self.trajectory_index}/{len(self.trajectory_points)} (Delay: {self.trajectory_delay*1000:.2f}ms, AnimSpeed: {self.animation_speed*100:.0f}%)")
        else:
            # 無法到達該點
            if self.skip_unreachable_points:
                # 檢查總跳過比例
                if self.skipped_points_count > len(self.trajectory_points) * 0.2:  # 超過20%
                ...
                while self.trajectory_index < len(self.trajectory_points):
                    ...
                    ok_next, msg_next = self.move_to(pt_next[0], pt_next[1], pt_next[2])
    ...
    def set_trajectory_speed(self, speed):
        """設定軌跡速度（同時調整延遲時間和關節動畫速度）
        speed: 1-1000，數字越大速度越快
        延遲時間範圍: 5ms(speed=1) 到 0.005ms(speed=1000)
        關節動畫速度也會同步提升
        """
        # 調整發送目標點的間隔時間
        self.trajectory_delay = 0.0005 / speed

        # 關節動畫速度提升，但設置上限為0.8避免過快導致不穩定
        # 使用對數增長，讓低速區間變化更明顯，高速區間趨於平緩
        if speed <= 20:
            # 1-20x: 線性增長從12%到約45%
            speed_multiplier = 1 + (np.sqrt(speed) - 1) * 0.8
        else:
            # 20-1000x: 對數增長，逐漸接近80%
            log_factor = np.log10(speed / 20) / np.log10(1000 / 20)  # 0到1之間
            speed_multiplier = 3.0 + log_factor * 3.67  # 從3.0增長到6.67

        self.animation_speed = min(0.8, self.base_animation_speed * speed_multiplier)
...
def start_gui():
    ...
    def move_arc():
        try:
            ...
            delay_ms = (0.005 / current_speed) * 1000
    ...
    tk.Label(lf, text="Speed").pack(pady=(5,0))
    spd = tk.Scale(lf, from_=5, to=1000, orient=tk.HORIZONTAL, length=180,
                   command=lambda v: setattr(ctrl, 'base_animation_speed', float(v)/100) if not ctrl.is_following_trajectory else None)
    spd.set(12)
    spd.pack()
    ...
    # 軌跡速度滑桿
    tk.Label(tf, text="").pack(pady=2)  # 分隔線

    def update_traj_speed(v):
        speed = float(v)
        ctrl.set_trajectory_speed(speed)
        delay_ms = (0.005 / speed) * 1000  # 轉換為毫秒
        anim_speed_percent = int(ctrl.animation_speed * 100)
        ...
        speed_label.config(text=f"Speed: {int(speed)}x ({delay_str}, Anim:{anim_speed_percent}%)")
    ...
    speed_label = tk.Label(speed_frame, text="Speed: 100x (0.05ms, Anim:36%)", width=30)
    speed_label.pack()

    traj_speed = tk.Scale(speed_frame, from_=1, to=1000, resolution=1,
                          orient=tk.HORIZONTAL, length=180,
                          command=update_traj_speed, showvalue=False)
    traj_speed.set(100)  # 預設速度為100x
    traj_speed.pack()

    # 初始化速度顯示
    update_traj_speed(100)

    tk.Label(speed_frame, text="(Range: 1-1000x, Max Anim: 80%)", font=("Arial", 7)).pack()

    # 跳過無法到達點的選項
    skip_frame = tk.Frame(tf)
    skip_frame.pack(fill=tk.X, padx=5, pady=5)

    skip_var = tk.BooleanVar(value=True)  # 預設開啟
