...
class AnimationController:
    def __init__(self):
        ...
        self.trajectory_start_marker = None  # 新增：軌跡起點標記
        self.trajectory_end_marker = None  # 新增：軌跡終點標記
        ...
        self.status_callback = None  # 新增：狀態更新回調
    ...
    def log_status(self, message):
        """記錄狀態訊息"""
        print(message)  # 仍然輸出到終端機
        if self.status_callback:
            self.status_callback(message)  # 同時更新GUI
    ...
    def show_trajectory(self, points):
        ...
        # 移除舊的起終點標記
        if self.trajectory_start_marker:
            vis.remove_geometry(self.trajectory_start_marker, reset_bounding_box=False)
        if self.trajectory_end_marker:
            vis.remove_geometry(self.trajectory_end_marker, reset_bounding_box=False)
        ...
        ls.paint_uniform_color([0, 1, 0])  # 綠色軌跡線
        self.trajectory_line = ls
        vis.add_geometry(ls, reset_bounding_box=False)

        # 添加起點標記（綠色小球）
        start_marker = o3d.geometry.TriangleMesh.create_sphere(radius=0.05)
        start_marker.paint_uniform_color([0, 1, 0])  # 綠色
        start_marker.compute_vertex_normals()
        start_marker.translate(points[0])
        self.trajectory_start_marker = start_marker
        vis.add_geometry(start_marker, reset_bounding_box=False)

        # 添加終點標記（藍色小球）
        end_marker = o3d.geometry.TriangleMesh.create_sphere(radius=0.05)
        end_marker.paint_uniform_color([0, 0, 1])  # 藍色
        end_marker.compute_vertex_normals()
        end_marker.translate(points[-1])
        self.trajectory_end_marker = end_marker
        vis.add_geometry(end_marker, reset_bounding_box=False)
    ...
    def clear_trajectory_markers(self):
        ...
        if self.trajectory_start_marker:
            vis.remove_geometry(self.trajectory_start_marker, reset_bounding_box=False)
            self.trajectory_start_marker = None
        if self.trajectory_end_marker:
            vis.remove_geometry(self.trajectory_end_marker, reset_bounding_box=False)
            self.trajectory_end_marker = None
    ...
    def move_next_point(self):
        ...
        if self.trajectory_index >= len(self.trajectory_points):
            if self.skipped_points_count > 0:
                self.log_status(f"✓ 軌跡完成！總點數: {len(self.trajectory_points)}, 跳過: {self.skipped_points_count}")
            else:
                self.log_status(f"✓ 軌跡完成！總點數: {len(self.trajectory_points)}")
            self.is_following_trajectory = False
            self.skipped_points_count = 0
            # 清除軌跡標記
        ...
        if ok:
            ...
            if self.trajectory_index % (
                    100 * self.trajectory_step) == 0 or self.trajectory_index % 100 < self.trajectory_step:
                delay_ms = self.trajectory_delay * 1000
                if delay_ms < 0.001:
                    delay_str = "No delay"
                else:
                    delay_str = f"{delay_ms:.2f}ms"
                actual_points = self.trajectory_index // self.trajectory_step
                self.log_status(
                    f"Progress: {actual_points}/{len(self.trajectory_points)} (Step: {self.trajectory_step}, Delay: {delay_str})")
        else:
            if self.skip_unreachable_points:
                if self.skipped_points_count > len(self.trajectory_points) * 0.2:
                    self.log_status(
                        f"✗ 不可達點過多 ({self.skipped_points_count}/{len(self.trajectory_points)}). 停止軌跡。")
                    self.is_following_trajectory = False
                    self.skipped_points_count = 0
                    self.clear_trajectory_markers()
                    return
                ...
                self.log_status(
                    f"⚠ 跳過不可達點 {first_skip_index}/{len(self.trajectory_points)}: {msg}\n  目標: ({pt[0]:.3f}, {pt[1]:.3f}, {pt[2]:.3f})")
                ...
                while self.trajectory_index < len(self.trajectory_points):
                    if self.skipped_points_count > len(self.trajectory_points) * 0.2:
                        self.log_status(
                            f"✗ 不可達點過多 ({self.skipped_points_count}/{len(self.trajectory_points)}). 停止。")
                        self.is_following_trajectory = False
                        self.skipped_points_count = 0
                        self.clear_trajectory_markers()
                        return
                    ...
                    if ok_next:
                        ...
                        self.trajectory_index += self.trajectory_step
                        self.log_status(
                            f"✓ 跳過 {consecutive_skips} 點後找到可達點 (目前 {self.trajectory_index}/{len(self.trajectory_points)})")
                        break
                    else:
                        self.skipped_points_count += 1
                        self.trajectory_index += self.trajectory_step
                        ...
                        if consecutive_skips % 10 == 0:
                            self.log_status(
                                f"⚠ 已連續跳過 {consecutive_skips} 點... (目前 {self.trajectory_index}/{len(self.trajectory_points)})")

                if not found_reachable:
                    self.log_status(f"⚠ 到達軌跡終點。總跳過: {self.skipped_points_count}")
                    self.is_following_trajectory = False
                    self.skipped_points_count = 0
                    self.clear_trajectory_markers()
                ...
                self.log_status(
                    f"✗ 失敗於點 {self.trajectory_index}/{len(self.trajectory_points)}: {msg}\n  目標: ({pt[0]:.3f}, {pt[1]:.3f}, {pt[2]:.3f})")
                current_pos = self.get_joint5_position()
                self.log_status(f"  當前: ({current_pos[0]:.3f}, {current_pos[1]:.3f}, {current_pos[2]:.3f})")
                self.is_following_trajectory = False
                self.skipped_points_count = 0
                self.clear_trajectory_markers()
...
class RobotControlGUI:
    ...
    def create_position_control(self, parent):
        ...
        self.status_text = tk.Text(status_frame, height=8, width=40,
                                   ...
                                   wrap=tk.WORD, yscrollcommand=status_scrollbar.set)
