...
class AnimationController:
    def __init__(self):
        ...
        self.trail_points = []  # 記錄已走過的點
        self.trail_complete_time = None  # 軌跡完成時間
    ...
    def animate(self):
        # 檢查是否需要清除殘影(軌跡完成後10秒)
        if self.trail_complete_time is not None:
            if time.time() - self.trail_complete_time > 10:
                self.clear_trail()
                self.trail_complete_time = None
    ...
    def update_trail(self):
        ...
        # 創建新的殘影線
        pts = o3d.utility.Vector3dVector(self.trail_points)
        lns = [[i, i + 1] for i in range(len(self.trail_points) - 1)]
    ...
    def move_next_point(self):
        ...
        if self.trajectory_index >= len(self.trajectory_points):
            ...
            self.skipped_points_count = 0
            # 記錄完成時間,用於10秒後清除殘影
            self.trail_complete_time = time.time()
            return
        ...
        if ok:
            # 記錄當前點到殘影列表
            self.trail_points.append([pt[0], pt[1], pt[2]])
            ...
            if self.skip_unreachable_points:
                ...
                while self.trajectory_index < len(self.trajectory_points):
                    ...
                    if ok_next:
                        ...
                        # 記錄當前點到殘影列表
                        self.trail_points.append([pt_next[0], pt_next[1], pt_next[2]])
