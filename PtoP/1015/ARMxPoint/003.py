...
class AnimationController:
    def __init__(self):
        ...
        self.trajectory_delay = 0.05  # 點與點之間的延遲時間（秒），預設50ms
    ...
    def move_next_point(self):
        ...
        if ok:
            ...
            if self.trajectory_index % 100 == 0:
                print(f"{self.trajectory_index}/{len(self.trajectory_points)}")
        else:
            print(f"Fail at {self.trajectory_index}")
            self.is_following_trajectory = False

    def set_trajectory_speed(self, speed):
        """設定軌跡速度（通過調整延遲時間）
        speed: 1-20，數字越大速度越快
        延遲時間範圍: 0.5秒(speed=1) 到 0.005秒(speed=20)
        """
        # 速度1時延遲0.5秒，速度20時延遲0.005秒
        # 使用對數刻度讓速度變化更平滑
        self.trajectory_delay = 0.5 / speed
...
def start_gui():
    ...
    def move_arc():
        try:
            ...
            delay_ms = (0.5 / current_speed) * 1000
            status.config(text=f"Arc: {length:.3f}m\nRunning... ({int(delay_ms)}ms/point)")
            root.update()
    ...
    spd = tk.Scale(lf, from_=5, to=30, orient=tk.HORIZONTAL, length=180,
                   command=lambda v: setattr(ctrl, 'animation_speed', float(v) / 100))
    spd.set(12)
    spd.pack()
    ...
    def update_traj_speed(v):
        ...
        delay_ms = (0.5 / speed) * 1000  # 轉換為毫秒
        speed_label.config(text=f"Speed: {int(speed)}x ({delay_ms:.0f}ms/point)")
        # 如果正在執行軌跡，更新狀態顯示
        if ctrl.is_following_trajectory:
            ...
            if "Running" in current_status:
                ...
                if len(lines) >= 1:
                    status.config(text=f"{lines[0]}\nRunning... ({delay_ms:.0f}ms/point)")

    speed_frame = tk.Frame(tf)
    speed_frame.pack(fill=tk.X, padx=5)

    speed_label = tk.Label(speed_frame, text="Speed: 10x (50ms/point)", width=22)
    speed_label.pack()

    traj_speed = tk.Scale(speed_frame, from_=1, to=20, resolution=1,
                          orient=tk.HORIZONTAL, length=180,
                          command=update_traj_speed, showvalue=False)
    traj_speed.set(10)  # 預設速度為10x (50ms/point)
    traj_speed.pack()

    # 初始化速度顯示
    update_traj_speed(10)

    tk.Label(speed_frame, text="(1=Slow 500ms, 20=Fast 25ms)", font=("Arial", 7)).pack()
