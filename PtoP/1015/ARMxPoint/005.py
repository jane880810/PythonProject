...
class AnimationController:
    ...
    def move_next_point(self):
        ...
        if ok:
            ...
            if self.trajectory_index % 100 == 0:
                print(
                    f"Progress: {self.trajectory_index}/{len(self.trajectory_points)} (Delay: {self.trajectory_delay * 1000:.2f}ms, AnimSpeed: {self.animation_speed:.2f})")
        else:
            print(f"Failed at point {self.trajectory_index}: {msg}")
            self.is_following_trajectory = False
            # 恢復基礎動畫速度
            self.animation_speed = self.base_animation_speed
    ...
    def set_trajectory_speed(self, speed):
        ...
        speed: 1-20，數字越大速度越快
        延遲時間範圍: 5ms(speed=1) 到 0.25ms(speed=20)
        關節動畫速度也會同步提升
        """
        # 調整發送目標點的間隔時間
        self.trajectory_delay = 0.005 / speed

        # 同時大幅提高關節動畫速度，讓關節能更快到達目標
        # 速度1x時用基礎速度0.12 (12%)
        # 速度20x時動畫速度提升到0.96 (96%)，提升8倍
        # 使用平方根增長曲線，讓速度提升更明顯
        speed_multiplier = 1 + (np.sqrt(speed) - 1) * 0.8
        self.animation_speed = min(0.99, self.base_animation_speed * speed_multiplier)
...
def start_gui():
    ...
    spd = tk.Scale(lf, from_=5, to=1000, orient=tk.HORIZONTAL, length=180,
                   command=lambda v: setattr(ctrl, 'base_animation_speed',
                                             float(v) / 100) if not ctrl.is_following_trajectory else None)
    spd.set(12)
    spd.pack()
    ...
    def update_traj_speed(v):
        ...
        speed_label.config(text=f"Speed: {int(speed)}x ({delay_ms:.2f}ms, Anim:{anim_speed_percent}%)")
        # 如果正在執行軌跡，更新狀態顯示
    ...
    speed_label = tk.Label(speed_frame, text="Speed: 5x (1.00ms, Anim:22%)", width=28)
    speed_label.pack()

    traj_speed = tk.Scale(speed_frame, from_=1, to=20, resolution=1,
                          orient=tk.HORIZONTAL, length=180,
                          command=update_traj_speed, showvalue=False)
    traj_speed.set(5)  # 預設速度為5x (1ms/point, 動畫速度提升到22%)
    traj_speed.pack()

    # 初始化速度顯示
    update_traj_speed(5)

    tk.Label(speed_frame, text="(Delay: 1=5ms, 10=0.5ms, 20=0.25ms)", font=("Arial", 7)).pack()
    ...
    bf = tk.Frame(tf)
    bf.pack(pady=5)
    tk.Button(bf, text="Move (Arc)", command=move_arc, width=12).pack(side=tk.LEFT, padx=2)
