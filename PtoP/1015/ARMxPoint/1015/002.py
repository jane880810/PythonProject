...
改善在手臂逆向運動後正向運動滑桿位置錯誤並無顯示問題
新增reset pose按鈕 新增關節角度顯示於滑桿上
問題:reset pose無法直接回到預設姿態(6軸關節皆為0)
...
def start_gui():
    ...
    for i, (name, mn, mx) in enumerate(jcfg):
        ...
        s.set(0)
        s.pack(side=tk.LEFT)
        ctrl.joint_scales.append(s)  # 儲存滑桿引用

    tk.Button(lf, text="Reset Pose", command=lambda: ctrl.reset_pose(), width=15).pack(pady=5)
    tk.Button(lf, text="Quit", command=quit_prog, width=15).pack(pady=5)
