...
class PyCharmHP60CViewer:
    ...
    def __init__(self):
        ...
        print("畫面中心橘色十字 = 座標原點(0,0,0)")
        print("綠點 = 人物中心，標籤格式: X(深度) Y(水平) Z(垂直) 單位:公分")
    ...
    def pixel_to_xyz(self, px, py, depth_cm):
        """
        針孔相機反投影，以畫面中心為原點
        X = 深度（公分），Y = 水平偏移（右正），Z = 垂直偏移（上正）
        """
        if self.fx is None or depth_cm is None:
            return None
        X = depth_cm
        Y = (px - self.cx) / self.fx * depth_cm
        Z = -((py - self.cy) / self.fy * depth_cm)  # 負號：像素向下=Z負
        return (X, Y, Z)
    ...
    def draw_origin_crosshair(self, img):
        ...
        cv2.putText(img, "(0,0,0)", (cx+8, cy-6),
...
def main():
    try:
        ...
        print("座標原點: 畫面中心 (0,0,0)")
        print("  X = 深度   (正=遠離相機，公分)")
        print("  Y = 水平   (正=右，負=左，公分)")
        print("  Z = 垂直   (正=上，負=下，公分)")
        print(f"  FOV設定: H={CAMERA_HFOV_DEG}°  V={CAMERA_VFOV_DEG}°")
        print("  提示: 設定 CAMERA_FX/FY/CX/CY 可提高精度")
        print("=" * 55)
