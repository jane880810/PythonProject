...
def start_gui():
    ...
    # 關節1控制
    tk.Label(root, text="關節1（p2~p8）繞Z軸", bg="#FFD9EC").pack()
    z_slider = tk.Scale(root, from_=-165, to=165, orient=tk.HORIZONTAL, length=300, command=on_z_slider)
    z_slider.pack(padx=20, pady=10)
    sliders['z_slider'] = z_slider

    # 關節2控制
    tk.Label(root, text="關節2（p3~p8 跟隨）繞Y軸", bg="#FFD9EC").pack()
    y_slider = tk.Scale(root, from_=-125, to=85, orient=tk.HORIZONTAL, length=300, command=on_y_slider)
    y_slider.pack(padx=20, pady=10)
    sliders['y_slider'] = y_slider

    # 關節3控制
    tk.Label(root, text="關節3（p4~p8 跟隨）繞Y軸", bg="#FFD9EC").pack()
    p4_slider = tk.Scale(root, from_=-55, to=180, orient=tk.HORIZONTAL, length=300, command=on_p4_y_slider)
    p4_slider.set(0)
    p4_slider.pack(padx=20, pady=10)
    sliders['p4_slider'] = p4_slider

    # 關節4控制
    tk.Label(root, text="關節4（p5~p8）繞P4的Z軸", bg="#FFD9EC").pack()
    p5_z_slider = tk.Scale(root, from_=-190, to=190, orient=tk.HORIZONTAL, length=300, command=on_p5_z_slider)
    p5_z_slider.pack(padx=20, pady=10)
    sliders['p5_z_slider'] = p5_z_slider

    # 關節5控制
    tk.Label(root, text="關節5（p6~p8）繞P5的Y軸", bg="#FFD9EC").pack()
    p6_y_slider = tk.Scale(root, from_=-25, to=205, orient=tk.HORIZONTAL, length=300, command=on_p6_y_slider)
    p6_y_slider.pack(padx=20, pady=10)
    sliders['p6_y_slider'] = p6_y_slider

    # 關節6控制
    tk.Label(root, text="關節6（p8）以自身為原點X軸自轉", bg="#FFD9EC").pack()
    p8_z_slider = tk.Scale(root, from_=-360, to=360, orient=tk.HORIZONTAL, length=300, command=on_p8_z_slider,
                           resolution=1)
    p8_z_slider.pack(padx=20, pady=10)
    sliders['p8_z_slider'] = p8_z_slider

    # 動畫速度控制
    tk.Label(root, text="動畫速度", bg="#FFD9EC").pack()

    def on_speed_change(value):
        anim_controller.animation_speed = float(value) / 100.0

    speed_slider = tk.Scale(root, from_=5, to=30, orient=tk.HORIZONTAL, length=300, command=on_speed_change)
    speed_slider.set(12)  # 預設值12%
    speed_slider.pack(padx=20, pady=10)

    # ===== 位置顯示區域 =====
    position_frame = tk.Frame(root, bg="#E6F3FF", relief=tk.RIDGE, bd=3)
    position_frame.pack(pady=10, padx=20, fill=tk.BOTH)

    tk.Label(position_frame, text="關節5 (P6) 當前位置 - 紅點標記",
             font=("Arial", 12, "bold"), bg="#E6F3FF", fg="#FF0000").pack(pady=5)

    # X座標顯示
    x_frame = tk.Frame(position_frame, bg="#E6F3FF")
    x_frame.pack(pady=2)
    tk.Label(x_frame, text="X: ", font=("Arial", 10), bg="#E6F3FF").pack(side=tk.LEFT)
    x_label = tk.Label(x_frame, text="0.000", font=("Arial", 10, "bold"),
                       bg="#FFFFFF", width=12, relief=tk.SUNKEN)
    x_label.pack(side=tk.LEFT, padx=5)

    # Y座標顯示
    y_frame = tk.Frame(position_frame, bg="#E6F3FF")
    y_frame.pack(pady=2)
    tk.Label(y_frame, text="Y: ", font=("Arial", 10), bg="#E6F3FF").pack(side=tk.LEFT)
    y_label = tk.Label(y_frame, text="0.000", font=("Arial", 10, "bold"),
                       bg="#FFFFFF", width=12, relief=tk.SUNKEN)
    ...
    z_frame.pack(pady=2)
    tk.Label(z_frame, text="Z: ", font=("Arial", 10), bg="#E6F3FF").pack(side=tk.LEFT)
    z_label = tk.Label(z_frame, text="0.000", font=("Arial", 10, "bold"),
                       bg="#FFFFFF", width=12, relief=tk.SUNKEN)
    ...
    reset_button = tk.Button(button_frame, text="回復預設姿態", command=reset_pose,
                           font=("Arial", 12, "bold"), bg="#B8E6B8", fg="#000000",
