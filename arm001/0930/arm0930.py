...
class AnimationController:
    ...
    def calculate_forward_kinematics(self, angles):
        ...
        R_p4_base = o3d.geometry.get_rotation_matrix_from_axis_angle([0, np.deg2rad(-90), 0])  # 初始-90度
    ...
    def inverse_kinematics(self, target_pos, initial_guess=None):
        ...
        if initial_guess is None:
            # 使用當前角度作為初始猜測
            initial_guess = self.current_angles[:4]  # 只需要前4個關節

        # 定義目標函數：最小化當前位置與目標位置的距離
    ...
    def move_to_position(self, target_x, target_y, target_z):
        ...
        if success and error < 0.01:  # 誤差小於1cm
            # 設定目標角度
            for i in range(4):
...
def start_gui():
    ...
    tk.Label(target_frame, text="目標位置控制 (逆向運動學)",
    ...
    def move_to_target():
        ...
        try:
            ...
            if success:
                ...
                result_label.config(
                    text=f"無法到達該位置 (誤差: {error * 1000:.1f}mm)",
                    fg="#CC0000"
                )
        except ValueError:
            result_label.config(text="請輸入有效的數值！", fg="#CC0000")
        except Exception as e:
            result_label.config(text=f"錯誤: {str(e)}", fg="#CC0000")

    move_button = tk.Button(target_frame, text="移動到目標位置", command=move_to_target,
                            font=("Arial", 9, "bold"), bg="#AADDFF", fg="#000000",
                            padx=10, pady=3, relief=tk.RAISED, bd=2)
    ...
    set_current_button = tk.Button(target_frame, text="使用當前位置", command=set_current_as_target,
                                   font=("Arial", 8), bg="#DDDDDD", fg="#000000",
