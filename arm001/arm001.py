...
p4_initial_center = p3_base_local_init + p3_length_vec_init

# 設定P4初始旋轉為-90度
initial_local_y = np.array([0, 1, 0])  # 初始P2坐標系的Y軸
R_world_initial = o3d.geometry.get_rotation_matrix_from_axis_angle(initial_local_y * np.deg2rad(-90))
T_initial = np.eye(4)
T_initial[:3, :3] = R_world_initial

node4_group.transform(T_initial, p4_initial_center)
...
def start_gui():
    ...
    # 關節3控制
    tk.Label(root, text="關節3（p4~p8 跟隨）繞Y軸 [0度=-90度實際]", bg="#FFD9EC").pack()
    p4_slider = tk.Scale(root, from_=-55, to=185, orient=tk.HORIZONTAL, length=300, command=on_p4_y_slider)
    p4_slider.set(0)
