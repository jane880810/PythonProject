...
class AnimationController:
    ...
    def update_joint(self, joint_idx, delta_angle):
        ...
        if joint_idx == 0:  # Joint 1
            ...
            p8_world_x = self.R_p2_total @ self.R_p3_total @ self.R_p4_total @ self.R_p5_total @ self.R_p6_total @ np.array([1, 0, 0])
    ...
    def inverse_kinematics_analytical(self, x4A, y4A, z4A):
        ...
        注意：根據實際模型座標系，需要進行座標轉換
        """
        try:
            # 座標系轉換：交換 X/Y 並反轉 X 的正負號
            x_model = -y4A  # 用戶的 Y → 模型的 -X
            y_model = x4A   # 用戶的 X → 模型的 Y
            z_model = z4A   # Z 保持不變
