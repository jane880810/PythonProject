...
class AnimationController:
    ...
    def inverse_kinematics_analytical(self, x4A, y4A, z4A):
        """
        解析解逆向運動學（根據公式 3.13-3.21）
        輸入: P4-A 點座標 (x4A, y4A, z4A)
        輸出: (θ1, θ2, θ3, θ4) 或 None
        """
        try:
            # 公式 3.13: θ1 計算
            theta1 = np.arctan2(-x4A, y4A)

            # 計算輔助變量
            r = np.sqrt(y4A ** 2 + x4A ** 2)  # 水平距離

            # 公式 3.14: θ2 計算
            z_offset = z4A - self.L1
            r_offset = r - self.S1
            ...
            # 餘弦定理計算角度
            cos_alpha = (self.L2 ** 2 + d ** 2 - (self.L3 ** 2 + self.S2 ** 2)) / (2 * self.L2 * d)

            # 檢查是否在有效範圍內
            if abs(cos_alpha) > 1.0:
                return None, "目標點超出工作範圍"
            ...
            # 公式 3.15: θ3 計算
            cos_gamma = (self.L2 ** 2 + self.L3 ** 2 + self.S2 ** 2 - d ** 2) / (
                        2 * self.L2 * np.sqrt(self.L3 ** 2 + self.S2 ** 2))
            ...
            # 公式 3.21: θ4 計算（簡化版，假設末端朝向）
            # 這裡可以根據需要設定末端姿態
            theta4 = 0  # 預設值，可根據需求調整
    ...
    def move_to_position_analytical(self, target_x, target_y, target_z):
        ...
        if error < 0.05:  # 5cm 誤差容許
            for i in range(4):
                self.set_target(i, angles[i])
            return True, error, angles, "成功"
        else:
            return False, error, angles, f"誤差過大: {error * 1000:.1f}mm"
...
# ---------- GUI 控制 ----------
def start_gui():
    def on_joint_slider(joint_idx, value):
        anim_controller.set_target(joint_idx, int(value))
    ...
    root = tk.Tk()
    root.title("RA605-710-GC 控制面板 - 解析解IK")
    ...
    for i, (label, min_v, max_v, default, key) in enumerate(joint_configs):
        ...
        frame.pack(pady=3)
        tk.Label(frame, text=label, bg="#FFD9EC", font=("Arial", 9)).pack()
        slider = tk.Scale(frame, from_=min_v, to=max_v, orient=tk.HORIZONTAL,
                          length=250, command=lambda v, idx=i: on_joint_slider(idx, v))
        slider.set(default)
        slider.pack()
        sliders[key] = slider
    ...
    tk.Label(right_frame, text="逆向運動學控制（解析解）",
    ...
    x_slider = tk.Scale(x_frame, from_=-0.8, to=0.8, resolution=0.001,
    ...
    tk.Label(y_frame, text="目標 Y (m)", font=("Arial", 9, "bold"),
             bg="#FFE6E6", width=12, anchor='w').pack(side=tk.LEFT)
    y_slider = tk.Scale(y_frame, from_=-0.8, to=0.8, resolution=0.001,
                        orient=tk.HORIZONTAL, length=300, bg="#FFCCCC")
    y_slider.set(0.0)
    ...
    tk.Label(z_frame, text="目標 Z (m)", font=("Arial", 9, "bold"),
             bg="#FFE6E6", width=12, anchor='w').pack(side=tk.LEFT)
    z_slider = tk.Scale(z_frame, from_=0.0, to=1.5, resolution=0.001,
                        orient=tk.HORIZONTAL, length=300, bg="#FFCCCC")
    z_slider.set(1.0)
    ...
    def move_to_target():
        ...
        if success:
            result_label.config(
                text=f"✓ {message}\n誤差: {error * 1000:.2f}mm\n"
                     f"θ1={angles[0]:.1f}° θ2={angles[1]:.1f}° "
                     f"θ3={angles[2]:.1f}° θ4={angles[3]:.1f}°",
                fg="#006600"
            )
            sliders['j1'].set(int(angles[0]))
            sliders['j2'].set(int(angles[1]))
            sliders['j3'].set(int(angles[2]))
            sliders['j4'].set(int(angles[3]))
        else:
    ...
    # ========== 底部按鈕 ==========
    bottom_frame = tk.Frame(root, bg="#FFD9EC")
    bottom_frame.pack(side=tk.BOTTOM, pady=10)

    reset_button = tk.Button(bottom_frame, text="回復預設姿態", command=reset_pose,
    ...
    print_button = tk.Button(bottom_frame, text="打印關節位置", command=print_all_positions,
                             font=("Arial", 9, "bold"), bg="#FFE6CC", padx=10, pady=5)
    print_button.pack(side=tk.LEFT, padx=5)

    exit_button = tk.Button(bottom_frame, text="結束程式", command=exit_program,
