...
class AnimationController:
    def __init__(self):
        ...
        # 幾何向量（使用 DH 參數）
        self.p2_base = np.array([0, 0, 0])  # Joint 1 基座
        self.p3_offset = np.array([-self.S1, 0, self.L1])  # Joint 2 位移
        self.p4_offset = np.array([0, 0, self.L2])  # Joint 3 位移
        self.p5_offset = np.array([self.S2, 0, 0])  # Joint 4 位移
        self.p6_offset = np.array([self.L3, 0, 0])  # Joint 5 位移
        self.p8_offset = np.array([self.L4, 0, 0])  # Joint 6 位移
    ...
    def get_joint5_position(self):
        ...
        position = self.R_p2_total @ (
                ...
                self.R_p3_total @ (
                        ...
                        self.R_p4_total @ (
                                self.p5_offset +
                                self.R_p5_total @ self.p6_offset
                        )
                )
        )
        return position
    ...
    def get_all_joint_positions(self):
        ...
        # Joint 2 (P2-A): 第一關節後
        positions['Joint_2'] = self.R_p2_total @ self.p3_offset

        # Joint 3 (P3-A): 第二關節後
        positions['Joint_3'] = self.R_p2_total @ (
                self.p3_offset +
                self.R_p3_total @ self.p4_offset
        )
        ...
        positions['Joint_4'] = self.R_p2_total @ (
                ...
                self.R_p3_total @ (
                        self.p4_offset +
                        self.R_p4_total @ self.p5_offset
                )
        )
        ...
        # Joint 6 (末端執行器): 第六關節
        positions['Joint_6'] = self.R_p2_total @ (
                self.p3_offset +
                self.R_p3_total @ (
                        self.p4_offset +
                        self.R_p4_total @ (
                                ...
                                self.R_p5_total @ (
                                        self.p6_offset +
                                        self.R_p6_total @ self.p8_offset
                                )
                        )
                )
        )
    ...
    def update_joint(self, joint_idx, delta_angle):
        ...
        if joint_idx == 0:  # 關節1 (Z軸)
            ...
            center = self.R_p2_total @ self.p3_offset
            T = np.eye(4)
            T[:3, :3] = R_world
            ...
            local_y = self.R_p2_total @ np.array([0, 1, 0])
            R_world = o3d.geometry.get_rotation_matrix_from_axis_angle(local_y * delta_rad)
            center = self.R_p2_total @ (self.p3_offset + self.R_p3_total @ self.p4_offset)
            T = np.eye(4)
            T[:3, :3] = R_world
            ...
            center = self.R_p2_total @ (self.p3_offset + self.R_p3_total @ (
                    self.p4_offset + self.R_p4_total @ self.p5_offset))
            T = np.eye(4)
            T[:3, :3] = R_world
            ...
            center = self.R_p2_total @ (self.p3_offset + self.R_p3_total @ (self.p4_offset + self.R_p4_total @ (
                    self.p5_offset + self.R_p5_total @ self.p6_offset)))
            T = np.eye(4)
            T[:3, :3] = R_world
            ...
            center = self.R_p2_total @ (self.p3_offset + self.R_p3_total @ (
                    self.p4_offset + self.R_p4_total @ (self.p5_offset + self.R_p5_total @ (
                    self.p6_offset + self.R_p6_total @ self.p8_offset))))
            T = np.eye(4)
            T[:3, :3] = R_world
