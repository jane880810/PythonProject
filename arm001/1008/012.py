...
座標系說明：
- 用戶座標系（GUI顯示）：X, Y, Z
- 內部計算座標系：x_internal, y_internal, z_internal
- 3D視圖座標系：與內部計算座標系相同
...
class AnimationController:
    ...
    def get_joint5_position(self):
        """計算關節5位置（用戶座標系，用於GUI顯示）"""
        position_internal = self.get_joint5_position_internal()
    ...
    def inverse_kinematics_analytical(self, x4A, y4A, z4A):
        ...
        try:
            # ===== 座標系轉換：用戶 → 內部 =====
            x_model = -y4A  # 內部 X = -用戶 Y
            y_model = x4A  # 內部 Y = 用戶 X
            z_model = z4A  # 內部 Z = 用戶 Z

            print(f"\n[IK Debug] 輸入座標（用戶）: X={x4A:.4f}, Y={y4A:.4f}, Z={z4A:.4f}")
            print(f"[IK Debug] 轉換座標（內部）: x={x_model:.4f}, y={y_model:.4f}, z={z_model:.4f}")
    ...
    def move_to_position_analytical(self, target_x, target_y, target_z):
        """使用解析解IK移動到目標位置"""
        print(f"\n{'=' * 60}")
        print(f"開始IK計算 - 目標位置（用戶座標）: X={target_x:.4f}, Y={target_y:.4f}, Z={target_z:.4f}")
        print(f"{'=' * 60}")
        ...
        print(f"[FK驗證] 計算位置: X={final_pos_user[0]:.4f}, Y={final_pos_user[1]:.4f}, Z={final_pos_user[2]:.4f}")
