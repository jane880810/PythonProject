...
class AnimationController:
    ...
    def get_joint5_position(self):
        ...
        # 轉換為用戶座標系：用戶(X,Y,Z) = 內部(Y,-X,Z)
        position_user = np.array([
            position_internal[1],  # 用戶 X = 內部 Y
            -position_internal[0],  # 用戶 Y = -內部 X
            position_internal[2]  # 用戶 Z = 內部 Z
        ])
    ...
    def move_to_position_analytical(self, target_x, target_y, target_z):
        ...
        # 轉換為用戶座標系進行比較：用戶(X,Y,Z) = 內部(Y,-X,Z)
        final_pos_user = np.array([
            final_pos_internal[1],  # 用戶 X = 內部 Y
            -final_pos_internal[0],  # 用戶 Y = -內部 X
            final_pos_internal[2]  # 用戶 Z = 內部 Z
        ])
