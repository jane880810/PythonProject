...
class AnimationController:
    ...
    def inverse_kinematics_analytical(self, x4A, y4A, z4A):
        """
        解析解逆向運動學（根據實際幾何參數修正）
        輸入: 目標點座標 (x4A, y4A, z4A)
        輸出: (θ1, θ2, θ3, θ4) 或 None
        """
        try:
            # 使用實際幾何參數
            # p3_base_local = [-0.03, 0, 0.38]
            # p3_length_vec = [0, 0, 0.34]
            # p4_length_vec = [0.04, 0, 0.09]
            # p5_length_vec = [0, 0, 0.24]

            offset_x = -0.03  # P2 到 P3 的 X 偏移
            offset_z = 0.38  # P2 到 P3 的 Z 偏移
            L2_actual = 0.34  # P3 長度
            L4_x = 0.04  # P4 X 偏移
            L4_z = 0.09  # P4 Z 偏移
            L5_z = 0.24  # P5 長度
            ...
            # 旋轉後的水平距離
            r = np.sqrt(y4A ** 2 + x4A ** 2)

            # 考慮 P4 初始 -90° 旋轉後，實際的手臂長度投影
            # P3→P4→P5 形成的鏈條
            z_from_base = z4A - offset_z  # 從 P3 基座開始的高度
            r_from_base = r - abs(offset_x)  # 從 P3 基座開始的水平距離

            # 計算 P4 段的有效長度（考慮 -90° 初始旋轉）
            L4_effective = np.sqrt(L4_x ** 2 + L4_z ** 2)  # ≈ 0.098

            # P3→P4→P5 的總長度鏈
            # 使用修正的幾何關係
            target_distance = np.sqrt(r_from_base ** 2 + z_from_base ** 2)

            # 使用餘弦定理計算 θ2
            # 三角形邊長：L2_actual, L4_effective+L5_z, target_distance
            arm_length = L4_effective + L5_z  # P4+P5 總長度

            cos_theta2_part = (L2_actual ** 2 + target_distance ** 2 - arm_length ** 2) / (
                        2 * L2_actual * target_distance)

            if abs(cos_theta2_part) > 1.0:
                return None, f"目標超出工作範圍 (距離: {target_distance:.3f}m)"

            angle_to_target = np.arctan2(z_from_base, r_from_base)
            angle_offset = np.arccos(cos_theta2_part)

            theta2 = angle_to_target + angle_offset - np.pi / 2

            # 使用餘弦定理計算 θ3
            cos_theta3 = (L2_actual ** 2 + arm_length ** 2 - target_distance ** 2) / (2 * L2_actual * arm_length)

            if abs(cos_theta3) > 1.0:
                return None, "目標超出工作範圍"
    ...
    def move_to_position_analytical(self, target_x, target_y, target_z):
        ...
        # 放寬誤差容許到 10cm（因為幾何模型與理論略有差異）
        if error < 0.10:
            for i in range(4):
                self.set_target(i, angles[i])
            return True, error, angles, "成功"
        else:
            # 即使誤差較大，仍然嘗試移動（可能是模型差異）
            for i in range(4):
                self.set_target(i, angles[i])
            return True, error, angles, f"已移動（模型誤差: {error * 1000:.1f}mm）"
...
def start_gui():
    ...
    info_label = tk.Label(target_frame,
                          text="註：3D模型幾何與理論參數略有差異，可能產生小誤差",
                          font=("Arial", 7), bg="#FFE6E6", fg="#666666")
    ...
    def move_to_target():
        ...
        if success:
            if error < 0.05:  # 5cm 內為良好
                color = "#006600"
                status = "✓"
            else:  # 5-10cm 為可接受但有警告
                color = "#CC6600"
                status = "⚠"
